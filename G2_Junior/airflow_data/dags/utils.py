...
def hash_nonpk_cols_sha1_df(df: pd.DataFrame, table: str) -> pd.DataFrame:
    ...
    res["hashed"] = res[nonpk_cols].apply(
        lambda row: hashlib.sha1("|".join(row.values.astype(str)).encode()).hexdigest(), axis=1)
    logging.debug(res.head(20), res.columns)

    return res[pk_cols + ["hashed"]]


def get_filepath(dag_id: str, table: str, etl_stage: str, execution_date: str, abs_path_dir: str, format: str,
                 conn_id: str = "") -> str:
