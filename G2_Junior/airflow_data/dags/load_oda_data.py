...
from utils import CONFIG, TARGET_CONN_ID, TARGET_HOOK, check_if_need_to_skip, get_ddl_from_conf, \
    hash_nonpk_cols_sha1_df, get_filepath
...
default_args = {
    ...
    "start_date": datetime(2024, 4, 1, 12, 0, 0, tzinfo=pendulum.timezone("UTC")),
    "retries": 1,
...
@dag(default_args=default_args, description='ETL pipeline to load data from staging layer to oda layer',
...
def load_oda_data():
    ...
    @task()
    def transform(filepath, table, **context) -> Tuple[str, Optional[str]]:
        ...
        if max_loaded_dttm:
            ...
            where {increment_col} = '{max_loaded_dttm}'::timestamp 
            ...
            if df_u_changed_rows.shape[0] > 0:
                df_u = pd.merge(df_u_dataset, df_u_changed_rows, how='inner',
                                on=pk_cols_without_scd2)  # filter, save only updated rows
